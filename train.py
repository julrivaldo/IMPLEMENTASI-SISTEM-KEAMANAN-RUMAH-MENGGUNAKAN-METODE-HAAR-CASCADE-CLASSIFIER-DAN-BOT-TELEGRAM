...
def collect_data(name, ids):
    ...
    def show_frame():
        ...
        # Adjusting parameters for better eye detection
...
def add_member_gui():
    ...
    button_collect = tk.Button(add_member_window, text="Masukkan Data",
...
def train():
    ...
    if faces:
        ...
        winsound.Beep(900, 1000)
        intructions.config(text="Train Wajah selesai!")
    else:
...
def identify():
    ...
    if not os.listdir(dataset_path):
        # Tampilkan gambar kosong jika dataset kosong
        intructions.config(text='maaf dataset kosong')
        return
    cap = cv2.VideoCapture(0)
    face_cascade = cv2.CascadeClassifier(
        cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        ...
        if len(faces) > 0:
            ...
            for x, y, w, h in faces:
                ...
                if label[1] < 50:
                    ...
                    if not image_sent:
                        cv2.putText(frm, "tidak dikenal", (x, y),
...
judul = tk.Label(
    frame1, text="IMPLEMENTASI PENGENALAN WAJAH DAN MATA \nDALAM SISTEM KEAMANAN RUMAH DENGAN METODE\n HAAR CASCADE DAN BOT TELEGRAM SEBAGAI INFORMASI", bg='#BBBC7C')
...
# Buttons
btn1 = tk.Button(frame1, text=' deteksi & \n simpan gambar', fg="black", bg='grey', command=add_member_gui, image=btn1_image, compound='left',
...
btn3 = tk.Button(frame1, text=' Hasil', fg='black', bg='grey', command=identify, image=btn3_image, compound='left',
                 width=150, height=40, font=btn_font)
...
btn6 = tk.Button(frame1, text=' Hasil\ngambar', fg='black', bg='grey', command=deteksi_akhir, image=btn6_image, compound='left',
                 width=150, height=40, font=btn_font)
